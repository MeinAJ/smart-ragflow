...
renderer.link = function(href, title, text) {
  ...
  if (citeMatch) {
    ...
    return `<a href="${href}" class="cite-link" data-preview="true" data-doc-title="${docTitle}" data-file-name="${fileName}" title="点击预览：${docTitle}">[引用${citeMatch[1]}]</a>`
  }
  // 普通链接
...
const handleCiteClick = (e) => {
  ...
  if (href) {
    // 使用原始文件名（包含扩展名）以便正确识别文件类型
    previewFile.value = { url: href, name: fileName }
    previewVisible.value = true
    console.log('[ChatMessage] Opening preview:', { url: href, name: fileName })
  }
}
